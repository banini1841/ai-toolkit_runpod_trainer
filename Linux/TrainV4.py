...
CONTAINER_DISK_GB = int(cfg["CONTAINER_DISK_GB"])
VOLUME_DISK_GB = 0
VOLUME_ID = ""
...
# --- Local Paths ---
LOCAL_CONFIG1_PATH = cfg["LOCAL_CONFIG1_PATH"]
LOCAL_DATASET_DIR = cfg["LOCAL_DATASET_DIR"]
LOCAL_DOWNLOAD_DIR = cfg["LOCAL_DOWNLOAD_DIR"]

# +++ Resume Training +++
LOCAL_RESUME_SOURCE_DIR = cfg["LOCAL_RESUME_SOURCE_DIR"]

max_pod_wait_time = 1000

# --- Linux-specific paths ---
LOCAL_ENV_FILE_PATH = os.path.join(SCRIPT_DIR, "atw.txt")
...
# Remote Paths
REMOTE_WORKSPACE_DIR = VOLUME_MOUNT_PATH
REMOTE_TOOLKIT_DIR = f"{REMOTE_WORKSPACE_DIR}/ai-toolkit"
REMOTE_ENV_FILE_PATH = f"{REMOTE_TOOLKIT_DIR}/.env"
REMOTE_DATASET_DIR = f"{REMOTE_WORKSPACE_DIR}/dataset"
REMOTE_CONFIG_DIR = f"{REMOTE_TOOLKIT_DIR}/config"
REMOTE_OUTPUT_DIR = f"{REMOTE_TOOLKIT_DIR}/output"

LOG_FILE = f"{REMOTE_WORKSPACE_DIR}/training_run.log"
REMOTE_PID_FILE = f"{REMOTE_WORKSPACE_DIR}/train.pid"
REMOTE_EXIT_FILE = f"{REMOTE_WORKSPACE_DIR}/train_exit"
REMOTE_TRAIN_SCRIPT = f"{REMOTE_WORKSPACE_DIR}/run_train.sh"
...
if not config_1_exists:
    ...
    exit(1)
CONFIG_FILE_1 = f"config/{os.path.basename(LOCAL_CONFIG1_PATH)}"
if config_2_exists:
    CONFIG_FILE_2 = f"config/{os.path.basename(LOCAL_CONFIG2_PATH)}"
else:
    CONFIG_FILE_2 = None
...
if not RUNPOD_API_KEY:
    ...
    print("Please set it before running the script (e.g., export RUNPOD_API_KEY='your_key')")
    exit(1)
try:
...
def run_command(cmd_string, check=True, capture=True, use_shell=True):
    """Runs a command string using subprocess."""
    ...
    try:
        ...
        print(f"ERROR: Command not found: Check if '{cmd_name}' is in PATH.")
        raise
...
def _ssh_base_args():
    """Return common SSH options as a string."""
...
def run_remote_command(remote_cmd, check=True):
    ...
    cmd_str = f"{SSH_EXE_PATH} {_ssh_base_args()} {shlex.quote(user_host)} {shlex.quote(remote_cmd)}"
...
def transfer_file_or_dir(local_path, remote_target_path, is_directory=False, upload=True, check=False):
    """Uploads or downloads using SCP."""
    if not POD_IP_OR_HOSTNAME or not POD_PORT:
    ...
    quoted_remote = shlex.quote(remote_target_path)
    remote_spec = f"{user_host}:{quoted_remote}"
...
def launch_monitoring_window():
    """Attempts to launch a monitoring pane/window for live log tailing."""
    if not all([POD_IP_OR_HOSTNAME, POD_PORT, SSH_KEY_PATH, LOG_FILE]):
    ...
    ssh_tail_cmd = (
        ...
        f"-i {SSH_KEY_PATH} {user_host} 'tail -f {LOG_FILE}'"
    )

    # Try tmux if we're inside a tmux session
    if os.environ.get('TMUX'):
        try:
            subprocess.Popen(['tmux', 'split-window', '-h', ssh_tail_cmd])
            print("Monitoring launched in tmux pane.")
            return
        except Exception as e:
            print(f"tmux split failed: {e}")

    # Otherwise just print the command for the user
    print(f"To monitor training, run in another terminal:\n  {ssh_tail_cmd}")


def sha256_of_remote_file(remote_file):
    """Get SHA256 of remote file via SSH."""
    stat_cmd = f"sha256sum {shlex.quote(remote_file)} 2>/dev/null | awk '{{print $1}}'"
    user_host = f"{POD_USER}@{POD_IP_OR_HOSTNAME}"
    known_hosts_file = os.path.join(tempfile.gettempdir(), "ssh_known_hosts")
    ssh_cmd = (
        ...
        f"-o UserKnownHostsFile={shlex.quote(known_hosts_file)} "
        f"-i {shlex.quote(SSH_KEY_PATH)} "
        f"{shlex.quote(user_host)} {shlex.quote(stat_cmd)}"
    )
...
def continuous_output_sync(poll_interval=60, optimizer_check_interval=2, optimizer_max_wait=20):
    ...
    Downloads optimizer safely: waits until remote hash stabilizes before downloading.
    ...
    while not sync_stop_event.is_set():
        try:
            ...
            ssh_cmd = (
                ...
                f"-o UserKnownHostsFile={shlex.quote(known_hosts_file)} "
                f"-i {shlex.quote(SSH_KEY_PATH)} "
                f"{shlex.quote(user_host)} {shlex.quote(find_cmd)}"
            )
            ...
            for remote_file in remote_files:
                ...
                # --- download checkpoint ---
                rel_path = os.path.relpath(remote_file, REMOTE_OUTPUT_DIR)
...
# --- Modify training config before upload ---
...
try:
            ...
            for port_map in runtime_info['ports']:
                if port_map.get('privatePort') == 22 and port_map.get('publicPort') and port_map.get('ip') and port_map.get('type') == 'tcp':
                    ...
                    if pod_ip and pod_ip == port_map.get('ip'):
                        POD_IP_OR_HOSTNAME = port_map['ip']  # Fixed: was pod_map (typo)
    ...
    if run_remote_command(f"touch {LOG_FILE}", check=False) != 0:
    ...
    for cmd in setup_commands:
        ...
        run_remote_command(cmd, check=True)
    ...
    transfer_file_or_dir(LOCAL_ENV_FILE_PATH, REMOTE_TOOLKIT_DIR, upload=True, check=True)
    src_path = f"{REMOTE_TOOLKIT_DIR}/{os.path.basename(LOCAL_ENV_FILE_PATH)}"
    ...
    if LOCAL_RESUME_SOURCE_DIR and os.path.isdir(LOCAL_RESUME_SOURCE_DIR):
        ...
        transfer_file_or_dir(LOCAL_RESUME_SOURCE_DIR, REMOTE_OUTPUT_DIR, is_directory=True, upload=True)
    ...
    transfer_file_or_dir(LOCAL_DATASET_DIR, REMOTE_DATASET_DIR, is_directory=True, upload=True)
    ...
    transfer_file_or_dir(LOCAL_CONFIG1_PATH, REMOTE_CONFIG_DIR, upload=True, check=True)
    if config_2_exists:
        transfer_file_or_dir(LOCAL_CONFIG2_PATH, REMOTE_CONFIG_DIR, upload=True)
    ...
    lora_output_remote = REMOTE_OUTPUT_DIR
    ...
    ssh_list_cmd = (
        ...
        f"-o UserKnownHostsFile={shlex.quote(known_hosts_file)} "
        f"-i {shlex.quote(SSH_KEY_PATH)} "
        f"{shlex.quote(user_host)} {shlex.quote(list_files_cmd)}"
    )
    ...
    for remote_file in remote_files:
        ...
        # Skip safetensors (handled by continuous sync)
        if remote_file.endswith(".safetensors"):
            continue

        # Compute local path
        rel_path = os.path.relpath(remote_file, lora_output_remote)
    ...
    if pod_was_created and POD_ID:
        if error_occurred:
            if not training_completed:
                ...
                terminate_decision = False
        elif not error_occurred and training_completed:
